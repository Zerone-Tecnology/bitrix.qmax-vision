...
if ( $_SERVER['REQUEST_METHOD']=='POST' && $_POST['FPRO_KEY']=='45gsiLab+1qUicK' )
{
	...
	$arSelect = Array('ID', 'NAME', 'IBLOCK_ID', 'IBLOCK_SECTION_ID', 'QUANTITY', 'PROPERTY_BRAND');
...
// ---------------------------------------------------------------------------------------------------- Ecommerce
	$arResult['CIBlockElement'] = $arItem;

	$res = CIBlockSection::GetByID($arItem['IBLOCK_SECTION_ID']);
	if($ob = $res->Fetch())
		$arResult['CIBlockSection'] = $ob;
// ---------------------------------------------------------------------------------------------------- Ecommerce
...
// ---------------------------------------------------------------------------------------------------- Ecommerce
			foreach ($basket as $basketItem)
			{
				...
				$arResult['CSaleBasket']['ID'] = $basketItem->getField('ID');
				$arResult['CSaleBasket']['NAME'] = $basketItem->getField('NAME');
				$arResult['CSaleBasket']['PRODUCT_ID'] = $basketItem->getField('PRODUCT_ID');
				$arResult['CSaleBasket']['PRICE'] = $basketItem->getField('PRICE');
				$arResult['CSaleBasket']['QUANTITY'] = $basketItem->getField('QUANTITY');
				$arResult['CSaleBasket']['CURRENCY'] = $basketItem->getField('CURRENCY');
				$arResult['CSaleBasket']['WEIGHT'] = $basketItem->getField('WEIGHT');
			}
// ---------------------------------------------------------------------------------------------------- Ecommerce
...
// ---------------------------------------------------------------------------------------------------- Ecommerce
			$arResult['CSaleOrder']['LID'] = $order->getField('LID');
			$arResult['CSaleOrder']['CANCELED'] = $order->getField('CANCELED');
			$arResult['CSaleOrder']['STATUS_ID'] = $order->getField('STATUS_ID');
			$arResult['CSaleOrder']['PRICE'] = $order->getField('PRICE');
			$arResult['CSaleOrder']['CURRENCY'] = $order->getField('CURRENCY');
			$arResult['CSaleOrder']['USER_ID'] = $order->getField('USER_ID');
			$arResult['CSaleOrder']['DISCOUNT_VALUE'] = $order->getField('DISCOUNT_VALUE');
			$arResult['CSaleOrder']['TAX_VALUE'] = $order->getField('TAX_VALUE');
			$arResult['CSaleOrder']['USER_DESCRIPTION'] = $order->getField('USER_DESCRIPTION');
			$arResult['CSaleOrder']['ADDITIONAL_INFO'] = $order->getField('ADDITIONAL_INFO');
// ---------------------------------------------------------------------------------------------------- Ecommerce
			...
			{
				$arResult['ORDER_ID'] = $arResult['orderid'] = $result->getId();
				Event::send(array(
