...
<div class="i_mt25">
	...
	<div class="i_actionlist">
		<?foreach($arResult['ITEMS'] as $k=>$e):
			...
			if($k>0):?>
				<div class="i_actionlist_hr"></div>
			<?endif?>
			<div class="i_actionlist_item" id="<?=$this->GetEditAreaId($e['ID']);?>">

				<?	// ----------------------------------------------------------------------------------------------------   PREVIEW_PICTURE
				if($arParams['DISPLAY_PICTURE']!='N' && is_array($e['PREVIEW_PICTURE']) && $k==0):?>
				...
				if($arParams['DISPLAY_DATE']!='N' && $e['DISPLAY_ACTIVE_FROM']):?>
					<div class="i_actionlist_date"><?=$e['DISPLAY_ACTIVE_FROM']?></div>
				<?endif
					// ----------------------------------------------------------------------------------------------------   DISPLAY_ACTIVE_FROM?>

				<?	// ----------------------------------------------------------------------------------------------------   NAME
				if($arParams['DISPLAY_NAME']!='N' && $e['NAME']):?>
					<div class="i_actionlist_name">
					<?if(!$arParams['HIDE_LINK_WHEN_NO_DETAIL'] || ($e['DETAIL_TEXT'] && $arResult['USER_HAVE_ACCESS'])):?>
		...
		<?endforeach?>
		<a href="<?=$arParams['I_TITLE_LINK']?>" class="i_newlist_all"><span><?=GetMessage('ACTION_ALL')?></span></a>
	</div>
