	...
	if($arParams['DISPLAY_TOP_PAGER'])
		...
		<div class="i_new_wide_content">
			...
			<? $i=0; foreach($arResult['ITEMS'] as $k=>$e):
				...
				if($k>0):?>
				<?endif?>
				<div class="i_wide_newlist_item <?if($item_count==1) echo 'i_wide_newlist_item_w100';
				elseif($item_count==2) echo 'i_wide_newlist_item_w50';
				else echo 'i_wide_newlist_item_w33';?>" id="<?=$this->GetEditAreaId($e['ID']);?>">

					<?	// ----------------------------------------------------------------------------------------------------   PREVIEW_PICTURE
					if($arParams['DISPLAY_PICTURE']!='N' && is_array($e['PREVIEW_PICTURE'])):?>
						<div class="i_newlist_wide_img">
						<?if(!$arParams['HIDE_LINK_WHEN_NO_DETAIL'] || ($e['DETAIL_TEXT'] && $arResult['USER_HAVE_ACCESS'])):?>
							<a title="<?=$e['PREVIEW_PICTURE']['TITLE']?>" href="<?=$e['DETAIL_PAGE_URL']?>" style="background-image:url(<?=$e['PREVIEW_PICTURE']['SRC']?>)">
								<?/*<img
									border="0"
					...
					if($arParams['DISPLAY_DATE']!='N' && $e['DISPLAY_ACTIVE_FROM']):?>
						<div class="i_newlist_wide_date"><?=$e['DISPLAY_ACTIVE_FROM']?></div>
					<?endif
					// ----------------------------------------------------------------------------------------------------   DISPLAY_ACTIVE_FROM?>

					<?	// ----------------------------------------------------------------------------------------------------   NAME
					if($arParams['DISPLAY_NAME']!='N' && $e['NAME']):?>
						<div class="i_newlist_wide_name">
							<?if(!$arParams['HIDE_LINK_WHEN_NO_DETAIL'] || ($e['DETAIL_TEXT'] && $arResult['USER_HAVE_ACCESS'])):?>
					...
					/*if($arParams['DISPLAY_PREVIEW_TEXT']!='N' && $e['PREVIEW_TEXT']):?>
						<div class="i_newlist_wide_text">
							<?if(!$arParams['HIDE_LINK_WHEN_NO_DETAIL'] || ($e['DETAIL_TEXT'] && $arResult['USER_HAVE_ACCESS'])):?>
								<a href="<?=$e['DETAIL_PAGE_URL']?>"><span class="ifont110"><?=$e['PREVIEW_TEXT']?></span></a>
							<?else:?>
								<span class="ifont110"><?=$e['PREVIEW_TEXT']?></span>
							<?endif?>
						</div>
					<?endif*/
					// ----------------------------------------------------------------------------------------------------   PREVIEW_TEXT?>
		...
		<div class="i_wide_newlist_all">
			<a href="<?=$arParams['I_TITLE_LINK']?>"><span><?=GetMessage('NEWS_ALL')?></span></a>
		</div>
