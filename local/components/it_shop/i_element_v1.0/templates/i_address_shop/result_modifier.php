...
if($arResult['ITEMS']):
	...
	foreach ($arResult['ITEMS'] as $k=>$e):
		...
		// PREVIEW
		if( $e['PRO']['I_ADDRESS_'.$l]['VALUE'] )
			$arResult['ITEMS'][$k]['PREVIEW_TEXT'] = $e['PRO']['I_ADDRESS_'.$l]['~VALUE']['TEXT'];

		$p_yandex = explode(',', $e['PRO']['YANDEX_MAP']['VALUE']);

		$arResult['MAP_PLACEMARKS'][$k]['lat'] = $p_yandex[0];
		$arResult['MAP_PLACEMARKS'][$k]['lng'] = $p_yandex[1];
