...
class CBitrixPersonalOrderListComponent extends CBitrixComponent
{
	...
	{
		$SITE_ID = Context::getCurrent()->getSite();// ilab
	...
	{
		$arImgFields = ["PREVIEW_PICTURE", "DETAIL_PICTURE"];
		$basketProperties = [$arImgFields];
	...
	protected function getBasketItemImg($product_id) {
		$IMG = [];
		...
		while($ob = $res->GetNextElement())
		{
			...
			$IMG[$arFields['ID']] = $arFields;
		}

		if($IMG[$product_id]['DETAIL_PICTURE'])
			$pick = $IMG[$product_id]['DETAIL_PICTURE'];
		else
			$pick = $IMG[$product_id]['PREVIEW_PICTURE'];

		unset($IMG);

		$img_path = CFile::GetPath($pick);
	...
	protected function obtainDataOrders()
	{
		...
		while ($basket = $listBaskets->fetch())
		{
			...
			$imgAr = $this->getBasketItemImg($basket['PRODUCT_ID']);
			$listOrderBasket[$basket['ORDER_ID']][$basket['ID']] = $basket;
			$listOrderBasket[$basket['ORDER_ID']][$basket['ID']]['PICTURE'] = $imgAr;
		}
