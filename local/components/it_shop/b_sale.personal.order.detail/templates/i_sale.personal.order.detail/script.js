...
(function() {
	BX.Sale.PersonalOrderComponent.PersonalOrderDetail = {
		...
		{
			var linkMoreOrderInformation = document.getElementsByClassName('sale-order-detail-about-order-inner-container-slide-read-more')[0];
			var linkLessOrderInformation = document.getElementsByClassName('sale-order-detail-about-order-inner-container-slide-read-less')[0];
			...
			BX.bind(linkMoreOrderInformation, 'click', function()
			{
				...
				//clientInformation.style.display = 'inline-block';// bitrix
				$('.sale-order-detail-about-order-inner-container-details').slideDown();// ilab
				linkMoreOrderInformation.style.display = 'none';
				linkLessOrderInformation.style.display = 'inline-flex';
			},this);
			BX.bind(linkLessOrderInformation, 'click', function()
			{
//				clientInformation.style.display = 'none';// bitrix
				$('.sale-order-detail-about-order-inner-container-details').slideUp();// ilab
				linkMoreOrderInformation.style.display = 'inline-flex';
			...
			Array.prototype.forEach.call(listShipmentWrapper, function(shipmentWrapper)
			{
				...
				BX.bindDelegate(shipmentWrapper, 'click', { 'class': 'sale-order-detail-show-link' }, BX.proxy(function()
				{
					...
					hideInformation.style.display = 'inline-flex';
					$('.sale-order-detail-payment-options-shipment-composition-map').slideDown();// ilab
					//detailShipmentBlock.style.display = 'block';// bitrix
				}, this));
				BX.bindDelegate(shipmentWrapper, 'click', { 'class': 'sale-order-detail-hide-link' }, BX.proxy(function()
				{
					showInformation.style.display = 'inline-flex';
					hideInformation.style.display = 'none';
					$('.sale-order-detail-payment-options-shipment-composition-map').slideUp();// ilab
					//detailShipmentBlock.style.display = 'none';// bitrix
				}, this));
			});
