...
<div class="i_or_title iprel">
	<span class="i_or_title_icon"></span>
		...
		else
		{
			...
			else
			{
				...
				<script type="text/javascript">
					...
					function ajaxResult(res)
					{
						...
						i_change_locations()// ilab
						il_aemail();// ilab
						i_order_account();// ------------------------------------------ проверка email
					...
					// change
					function i_change_locations()
					{
						setTimeout(function() {
							$('body #ORDER_FORM input, body #ORDER_FORM select').styler();
						}, 100);
					}

					// refresh
					function i_refresh_locations()
					{
						setTimeout(function() {
							$('body #ORDER_FORM input, body #ORDER_FORM select').trigger('refresh');
						}, 100);
					}

					function i_order_account()
					{
				...
				<?if($_POST["is_ajax_post"] != "Y")
			{
				...
				<div id="order_form_content">
					...
					{
						foreach(array_unique($arResult["ERROR"]) as $v)
							echo ShowError($v);
						?>
						<script type="text/javascript">
