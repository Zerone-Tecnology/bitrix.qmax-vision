...
if(count($arResult["PERSON_TYPE"]) > 1)
{
	...
	<div class="section i_or_section i_or_person_type">
		...
		<div class="i_label_div">
			<?foreach($arResult["PERSON_TYPE"] as $v):?>
				<div class="label <?if ($v['CHECKED']=='Y')echo ' i_or_checked'?>">
					<input type="radio" id="PERSON_TYPE_<?=$v["ID"]?>" name="PERSON_TYPE" value="<?=$v["ID"]?>"<?if ($v["CHECKED"]=="Y") echo " checked=\"checked\"";?> onClick="submitForm()"> <label for="PERSON_TYPE_<?=$v["ID"]?>"><span><?=$v["NAME"]?></span></label>
				</div>
			<?endforeach?>
		</div>
