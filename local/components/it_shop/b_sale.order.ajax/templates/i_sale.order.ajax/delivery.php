...
<div class="bx_section i_or_section i_or_delivery">
	<?
	if(!empty($arResult["DELIVERY"]))
	{
		...
		foreach ($arResult["DELIVERY"] as $delivery_id => $arDelivery)
		{
			...
			if ($delivery_id !== 0 && intval($delivery_id) <= 0)
			{
				...
				{
					?>
					<div class="bx_block w100 vertical<?if($arProfile['CHECKED'] == 'Y')echo ' i_or_activ'?>">
						<div class="bx_element">
							...
							<label id="<?=$delivery_id?>" class="" for="ID_DELIVERY_<?=$delivery_id?>_<?=$profile_id?>">

								<?
								if( file_exists($_SERVER['DOCUMENT_ROOT'].SITE_TEMPLATE_PATH.'/ilab/img/svg/delivery/delivery'.$arDelivery["SID"].'.svg') )
									$deliveryImgURL = '/local/templates/ilab_it_shop/ilab/img/svg/delivery/delivery'.$arDelivery["SID"].'.svg';
								elseif (count($arDelivery["LOGOTIP"]) > 0)
								{
								...
								<div class="bx_logotype" onclick="BX('ID_DELIVERY_<?=$delivery_id?>_<?=$profile_id?>').checked=true;<?=$extraParams?>submitForm();">
									<span class="<?=$arDelivery["SID"]?>" style='background-image:url(<?=$deliveryImgURL?>);'></span>
								</div>
								...
								<div class="bx_description">
									...
									<span class="bx_result_price"><!-- click on this should not cause form submit -->
										...
										if($arProfile["CHECKED"] == "Y" && doubleval($arResult["DELIVERY_PRICE"]) > 0):
											...
											<div><?$APPLICATION->IncludeComponent('bitrix:sale.ajax.delivery.calculator', '', array(
			...
			else // stores and courier
			{
				...
				?>
				<div class="bx_block w100 vertical<?if($arDelivery['CHECKED'] == 'Y')echo ' i_or_activ'?>">
					...
					<div class="bx_element">
						...
						<label for="ID_DELIVERY_ID_<?=$arDelivery["ID"]?>" <?=$clickHandler?>>
							...
							<?
							if( file_exists($_SERVER['DOCUMENT_ROOT'].SITE_TEMPLATE_PATH.'/ilab/img/svg/delivery/delivery'.$arDelivery["ID"].'.svg') )
								$deliveryImgURL = '/local/templates/ilab_it_shop/ilab/img/svg/delivery/delivery'.$arDelivery["ID"].'.svg';
							elseif (count($arDelivery["LOGOTIP"]) > 0)
							{
								$arFileTmp = CFile::ResizeImageGet(
									$arDelivery["LOGOTIP"]["ID"],
							...
							<div class="bx_logotype"><span class="<?='delivery'.$arDelivery["ID"]?>" style='background-image:url(<?=$deliveryImgURL?>);'></span></div>

							<div class="bx_description">
								<div class="bx_description_title"><strong><?= htmlspecialcharsbx($arDelivery["NAME"])?></strong></div>
									<span class="bx_result_price">
										...
										<?if($arDelivery["PRICE"] > 0):?>
                                            <div><?=GetMessage("SALE_DELIV_PRICE");?>: <b><?=$arDelivery["PRICE_FORMATED"]?></b></div><br>
										<?endif;?>
									</span>
								<p>
									<?
