...
function updateBasketTable(basketItemId, res)
{
	...
	if (res.hasOwnProperty('WARNING_MESSAGE'))
	{
		...
		BX('warning_message').innerHTML = '<div class="i_b_warning_message">'+warningText+'</div>';
	}
...
{
	// ilab
	if( $('.i_ba_ss_check').length && !$('.i_ba_ss_check').is(':checked'))
	{
		$('.jq_ba_m_steaker_status').fadeIn();
		return false;	
	}
	else if( $('.jq_bai_terms_of_use').length && !$('.jq_bai_terms_of_use').is(':checked'))
	{
		$('.jq_ba_terms_of_use').fadeIn();
		return false;
	} else {
		$('.jq_ba_terms_of_use, .jq_ba_m_steaker_status').hide();
		// bitrix
