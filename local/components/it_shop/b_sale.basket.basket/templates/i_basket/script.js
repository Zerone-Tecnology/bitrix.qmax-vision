...
function skuPropClickHandler(e)
{
    ...
    if (!!target && target.hasAttribute('data-value-id'))
    {
        ...
        BX.ajax({
            url: '/local/components/it_shop/b_sale.basket.basket/ajax.php',
            method: 'POST',
            data: postData,
            dataType: 'json',
...
// ----------------------------------------------------------------------------------------------- iLab
            ...
            {
                var item_CURRENT_PRICE = item.PRICE_FORMATED.split(/\s+(?=\D+)/);

                BX('current_price_' + id).innerHTML = item_CURRENT_PRICE[0]+'<span> '+item_CURRENT_PRICE[1]+'</span>';
            }

            if (BX('old_price_' + id))
            {
                var item_OLD_PRICE = item.FULL_PRICE_FORMATED.split(/\s+(?=\D+)/);

                BX('old_price_' + id).innerHTML = (item.FULL_PRICE_FORMATED != item.PRICE_FORMATED ? item_OLD_PRICE[0]+'<span> '+item_OLD_PRICE[1]+'</span>' : '');

            }

            if (BX('sum_' + id))
            {
                var item_SUM = item.SUM.split(/\s+(?=\D+)/);

                BX('sum_' + id).innerHTML = item_SUM[0]+'<span> '+item_SUM[1]+'</span>';
            }
// ----------------------------------------------------------------------------------------------- OLD Bitrix
			/*
			 var item = res.BASKET_DATA.GRID.ROWS[id];
...
// ----------------------------------------------------------------------------------------------- OLD Bitrix
    ...
    {
        var allWeight_FORMATED = res.BASKET_DATA.allWeight_FORMATED.split(/\s+(?=\D+)/);
        BX('allWeight_FORMATED').innerHTML = allWeight_FORMATED[0]+'<span> '+allWeight_FORMATED[1]+'</span>';
    }

    if (BX('allSum_wVAT_FORMATED'))
    {
        var allSum_wVAT_FORMATED = res.BASKET_DATA.allSum_wVAT_FORMATED.split(/\s+(?=\D+)/);
        BX('allSum_wVAT_FORMATED').innerHTML = allSum_wVAT_FORMATED[0]+'<span> '+allSum_wVAT_FORMATED[1]+'</span>';
    }

    if (BX('allVATSum_FORMATED'))
    {
        var allVATSum_FORMATED = res.BASKET_DATA.allVATSum_FORMATED.split(/\s+(?=\D+)/);
        BX('allVATSum_FORMATED').innerHTML = allVATSum_FORMATED[0]+'<span> '+allVATSum_FORMATED[1]+'</span>';
    }

    if (BX('allSum_FORMATED'))
    {
        var allSum_FORMATED = res.BASKET_DATA.allSum_FORMATED.split(/\s+(?=\D+)/);
        BX('allSum_FORMATED').innerHTML = allSum_FORMATED[0]+'<span> '+allSum_FORMATED[1]+'</span>';
    }

    if (BX('PRICE_WITHOUT_DISCOUNT'))
    {
        var PRICE_WITHOUT_DISCOUNT = res.BASKET_DATA.PRICE_WITHOUT_DISCOUNT.split(/\s+(?=\D+)/);
        BX('PRICE_WITHOUT_DISCOUNT').innerHTML = (res['BASKET_DATA']['PRICE_WITHOUT_DISCOUNT'] != res['BASKET_DATA']['allSum_FORMATED'] ? PRICE_WITHOUT_DISCOUNT[0]+'<span> '+PRICE_WITHOUT_DISCOUNT[1]+'</span>' : '');
    }
...
{
    // ilab
    if( $('.jq_bai_terms_of_use').length && !$('.jq_bai_terms_of_use').is(':checked'))
    {
        $('.jq_ba_terms_of_use').fadeIn();
        return false;
    } else {
        $('.jq_ba_terms_of_use').hide();
        // bitrix
