	...
	<form method="post"  class="col-md-12 sale-profile-detail-form" action="<?=POST_FORM_ACTION_URI?>" enctype="multipart/form-data">
		...
		<div class="form-groups">
			<div class="form-group">
				...
				<div class="col-md-12" style="height:40px;display:flex;align-items:center;">
					<span class="i_prd_color"><?=$arResult["PERSON_TYPE"]["NAME"]?></span>
				</div>
			</div>		
			<div class="form-group">
		...
		foreach($arResult["ORDER_PROPS"] as $block)
		{
			if (!empty($block["PROPS"]))
			{
				...
				<div class="sale-personal-profile-detail-form-props">
					<?
					foreach($block["PROPS"] as $key => $property)
					{
						...
						<div class="form-group sale-personal-profile-detail-property-<?=strtolower($property["TYPE"])?>">
							...
							<div class="col-md-12"<?if($property["TYPE"]=='LOCATION')echo ' style="height:40px;display:flex;align-items:center;"'?>">
								<?
								...
								elseif ($property["TYPE"] == "LOCATION")
								{
									...
									else
									{
										...
										// ilab code
										foreach ($property['VALUES'] as $arVariant)
											if($currentValue==$arVariant['ID'])
											{
												$value		= $arVariant['ID'];
												$showvalue	= $arVariant['CITY_NAME_LANG'];
												break;
											}?>

										<span class="i_prd_color"><?=$showvalue?></span>

										<div class="i_location_comp">
											<?CSaleLocation::proxySaleAjaxLocationsComponent(
												array(
													"AJAX_CALL" => "N",
													'CITY_OUT_LOCATION' => 'Y',
		...
		?>
		<div class="sale-personal-profile-detail-form-props">
			<div class="form-group sale-personal-profile-detail-property-btn">
				<div></div>
				<div class="sale-personal-profile-btn-block<?/* col-md-offset-3 col-sm-9*/?>">
