...
$arParams['CAPTCHACode'] = $cpt->GetSID();

$frame = $this->createFrame('jqm_review', false)->begin();
...
if( $USER->GetLogin() )
    ...
    <form name="reviews" action="/local/templates/ilab_it_shop/ilab/ajax/reviews_add.php" method="POST" enctype="multipart/form-data">
        ...
        <table>
            ...
            <tr>
                <td>
                    <input type="hidden" name="captcha_sid" value="<?=$arParams['CAPTCHACode'];?>" />
                    <img class="jq_captcha" src="/bitrix/tools/captcha.php?captcha_sid=<?=htmlspecialcharsbx($arParams['CAPTCHACode'])?>" width="180" height="40" title="CAPTCHA" alt="CAPTCHA" jq_code="<?=htmlspecialcharsbx($arParams['CAPTCHACode'])?>"><img class="fi_recaptcha jq_recaptcha" src="<?=SITE_TEMPLATE_PATH?>/ilab/img/recaptcha.gif">
                    <br>
					<?=GetMessage('CHARACTERS')?>
                </td>
            </tr>
            <tr>
                <td>
            ...
            <tr>
                <td>
                    ...
                    <div class="jq_again idnone">
                        <font color="#fb4e00"><?=GetMessage('REVIEW_SENT')?></font><br><a href="javascript:void(0)" class="i_dash jq_click_again_review"><?=GetMessage('I_AGAIN')?></a>
                    </div>
                </td>
            </tr>
        </table>
        <input type="hidden" class="jq_product_ajax" value="" name="product_ajax">
        <input type="hidden" class="jq_approval_reviews" value="" name="approval_reviews">
