...
*/?>
			...
			<?if( $arParams['I_CHECK_QUANTITY']=='Y' && $arParams['I_QUAN_VERY_LITTLE'] && $arParams['I_QUAN_LITTLE'] && $arParams['I_QUAN_AVERAGE'] ):?>
				<div class="i_quantity aclear">
					<span class="i_quan_tx ifleft"><?=GetMessage('CATALOG_QUANTITY')?><span class="i_quan_sl"></span><span class="i_quantity_text"><?=$arResult['I_QUAN_TEXT']?></span></span>
				</div>
			<?endif?>
			<!-- Количество -->
	...
	<div class="i_additional_info jq_additional_info iprel">
		...
		<select class="i_offers_sl_mobile j_offers_sl_mobile idn">
			<option jq_ai_but="1" data-class="i_ai_des"><span class="i_ai_name"><?=GetMessage('DESCRIPTION')?></span></option>
			<?if( $arParams['I_INSTRUCTION']=='Y' ):?>
				<option jq_ai_but="2" data-class="i_ai_ins">
					<span class="i_ai_name"><?=GetMessage('INSTRUCTION')?></span>
					<?if( $arResult['PROPERTIES']['I_INSTRUCTION']['VALUE'] ):?>
						<span class="i_ai_count"><?=count( $arResult['PROPERTIES']['I_INSTRUCTION']['VALUE'] )?></span>
					<?endif?>
			...
			<?endif?>
			<option data-class="i_ai_fea" jq_ai_but="3"><span class="i_ai_name"><?=GetMessage('FEATURES')?></span></option>
			<option data-class="i_ai_br" jq_ai_but="4"><span class="i_ai_name"><?=GetMessage('A_BRAND')?></span></option>
			<option data-class="i_ai_vid" jq_ai_but="5">
				<span class="i_ai_name"><?=GetMessage('VIDEO')?></span>
				<?if( $arResult['PROPERTIES']['I_VIDEO']['VALUE'] ):?>
					<span class="i_ai_count"><?=count( $arResult['PROPERTIES']['I_VIDEO']['VALUE'] )?></span>
				<?endif?>
			</option>
			<option data-class="i_ai_rev" jq_ai_but="6">
				<span class="i_ai_name"><?=GetMessage('REVIEWS')?></span>
				<?if( $arResult['I_REVIEWS'] ):?>
					<span class="i_ai_count jq_re_count"><?=count($arResult['I_REVIEWS'])?></span>
				<?endif?>
		...
		<div class="i_offers_sl_mobile_bottom_block">
			<select class="i_offers_sl_mobile i_offers_sl_mobile_bottom j_offers_sl_mobile idn">
				<option jq_ai_but="1" data-class="i_ai_des"><span class="i_ai_name"><?=GetMessage('DESCRIPTION')?></span></option>
				<?if( $arParams['I_INSTRUCTION']=='Y' ):?>
					<option jq_ai_but="2" data-class="i_ai_ins">
						<span class="i_ai_name"><?=GetMessage('INSTRUCTION')?></span>
