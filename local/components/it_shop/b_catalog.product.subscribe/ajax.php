...
// --------------------------------------------------------------------------------------------------- ilab
					...
					$res = CIBlockElement::GetByID($_POST['itemId']);
					if ($ar_res = $res->GetNext())
					{
						$item_name = $ar_res['NAME'];
						$item_url = $ar_res['DETAIL_PAGE_URL'];
					}

					$arFields = array(
						'USER_NAME' => $USER->GetFullName() ? $USER->GetFullName() : 'Зарегистрированный пользователь',
						'EMAIL_TO' => $userContact,
						'NAME' => $item_name,
						'PAGE_URL' => $item_url,
						//'UNSUBSCRIBE_URL' => $subscribeManager->unSubscribe($subscribeData)
					);
					//echo CEvent::Send('CATALOG_PRODUCT_SUBSCRIBE_MANAGER', $_POST['siteId'], $arFields, 'N', '').' | work[subscribe] == Y';
					CEvent::Send('CATALOG_PRODUCT_SUBSCRIBE_MANAGER', $_POST['siteId'], $arFields, 'N', '');

					/*echo '<pre>';
						echo $_POST['siteId'].' | '.print_r($arFields);
					echo '</pre>';*/
// --------------------------------------------------------------------------------------------------- ilab
					echo Bitrix\Main\Web\Json::encode(
...
// --------------------------------------------------------------------------------------------------- ilab
				...
				$arFields = array(
					'USER_NAME' => 'Гость',
					'EMAIL_TO' => $contact['user'],
					'NAME' => $item_name,
					'PAGE_URL' => $item_url,
					//'UNSUBSCRIBE_URL' => $subscribeManager->unSubscribe($subscribeData)
				);
				//echo CEvent::Send('CATALOG_PRODUCT_SUBSCRIBE_MANAGER', $_POST['siteId'], $arFields, 'N', '').' | work[contactFormSubmit] == Y';
				CEvent::Send('CATALOG_PRODUCT_SUBSCRIBE_MANAGER', $_POST['siteId'], $arFields, 'N', '');
