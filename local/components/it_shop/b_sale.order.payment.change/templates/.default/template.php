						...
						foreach ($arResult['PAYSYSTEMS_LIST'] as $key => $paySystem)
						{
							...
							<div class="sale-order-payment-change-pp-company">
								<div class="sale-order-payment-change-pp-company-graf-container">
									...
									>
									<?
									if(file_exists($_SERVER['DOCUMENT_ROOT'].'/local/templates/ilab_it_shop/ilab/img/svg/payment/payment'.$paySystem['ID'].'.svg'))
										$paySystem['LOGOTIP'] = '/local/templates/ilab_it_shop/ilab/img/svg/payment/payment'.$paySystem['ID'].'.svg';
									elseif (empty($paySystem['LOGOTIP']))
