...
if (is_object($cp))
{
	...
	if(empty($arResult['ERRORS']['FATAL']))
	{
		...
		foreach($arResult['BASKET'] as $e)
			if( $e['PREVIEW_PICTURE'] || $e['DETAIL_PICTURE'] )
				$arResult['I_PRODUCT_ITEMS'][$e['PRODUCT_ID']]['PICTURE'] = $e['PICTURE']['SRC'];
			else
			{
				$bask_id[]	= $e['PRODUCT_ID'];
				if( $e['PARENT'] )
					$bask_id[]	= $e['PARENT']['ID'];
			}
		...
		if( $bask_id )
		{
			$arSelect = Array('ID', 'NAME', 'IBLOCK_ID', 'QUANTITY', 'PROPERTY_MORE_PHOTO');
			$arFilter = Array('ID'=>$bask_id);
			...
			while($ob = $res->GetNextElement())
			{
				...
				if( $obj['PROPERTY_MORE_PHOTO_VALUE'] )
					$arResult['I_PRODUCT_ITEMS'][$obj['ID']]['PICTURE']		= CFile::GetPath($obj['PROPERTY_MORE_PHOTO_VALUE']);
			}
		}
